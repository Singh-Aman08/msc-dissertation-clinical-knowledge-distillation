...
ADAPTER_PATH = "weighted_sft_qwen25_1.5b/checkpoint-150"
INPUT_FILE = "testing_consultations_06.jsonl"
OUTPUT_FILE = "patient_report_weighted_sft_qwen25_1.5b_part2_150_06.jsonl"
...
# No report output is required for the benchmark
EFFICIENCY_OUTPUT_FILE = "weighted_sft_computational_efficiency.csv"
...
print("\n" + "=" * 80)
print("WEIGHTED SFT COMPUTATIONAL EFFICIENCY BENCHMARK")
print("=" * 80)

print(f"Total parameters:      {total_parameters:,}")
print(f"Total parameters (B):  {total_parameters / 1e9:.3f} B")
print(f"Trainable parameters:  {trainable_parameters:,}")
print(f"Trainable parameters (M): {trainable_parameters / 1e6:.2f} M")
...
with open(INPUT_FILE, "r", encoding="utf-8") as infile:
    ...
    for line in infile:
        ...
        if len(records) >= N_BENCHMARK:
            break


print(f"\nLoaded {len(records)} consultations for benchmarking.")
...
#
# Paste your ORIGINAL `messages = [...]` block below.
#
# The only thing that should change is that the prompt is
# generated inside the benchmark loop for each consultation.
#
# ============================================================
...
# ============================================================
#
# Warm-up is NOT included in the benchmark.
# This prevents model-loading / first-generation overhead
# from contaminating the latency measurements.
# ============================================================

print("\nRunning warm-up generation...")

warmup_transcript = records[0].get(
    "consultation",
    ""
)

warmup_messages = build_messages(warmup_transcript)
...
for i, record in enumerate(records, start=1):
    ...
    generated_tokens.append(n_output_tokens)
    tokens_per_second.append(tok_per_sec)
    peak_memories.append(peak_memory_gb)


    # --------------------------------------------------------
    # PRINT PER-REPORT RESULT
    # --------------------------------------------------------

    print(
        f"Report {i:02d} | "
        f"Input tokens: {input_token_count} | "
        f"Output tokens: {n_output_tokens} | "
        f"Latency: {latency:.3f}s | "
        f"Tokens/s: {tok_per_sec:.2f} | "
        f"Peak GPU: {peak_memory_gb:.2f} GB"
    )
...
sd_latency = np.std(latencies, ddof=1)

mean_tokens_per_second = np.mean(tokens_per_second)
sd_tokens_per_second = np.std(
    tokens_per_second,
    ddof=1
)
...
print(
    f"\nParameters:"
    f"\n  Total: {total_parameters / 1e9:.3f} B"
    f"\n  Trainable: {trainable_parameters / 1e6:.2f} M"
)

print(
    f"\nPeak GPU memory:"
    f"\n  Mean: {mean_peak_memory:.2f} GB"
    f"\n  Maximum: {max_peak_memory:.2f} GB"
)

print(
    f"\nLatency per report:"
    f"\n  Mean: {mean_latency:.3f} s"
    f"\n  SD: {sd_latency:.3f} s"
)

print(
    f"\nGeneration speed:"
    f"\n  Mean: {mean_tokens_per_second:.2f} tokens/s"
    f"\n  SD: {sd_tokens_per_second:.2f} tokens/s"
)

print(
    f"\nGenerated tokens:"
    f"\n  Mean: {mean_generated_tokens:.1f}"
)
...
benchmark_results = pd.DataFrame({
    ...
    "peak_gpu_memory_GB": peak_memories

})


benchmark_results.to_csv(
    EFFICIENCY_OUTPUT_FILE,
    index=False
)
...
summary = pd.DataFrame({
    ...
    "metric": [
        ...
        "Maximum peak GPU memory (GB)",
        "Mean latency per report (s)",
        "SD latency (s)",
        "Mean tokens per second",
        "SD tokens per second",
...
summary.to_csv(
    "weighted_sft_computational_efficiency_summary.csv",
    index=False
)


print("\nSaved:")
print("1. weighted_sft_computational_efficiency.csv")
print("2. weighted_sft_computational_efficiency_summary.csv")