import json
import os
from vllm import LLM, SamplingParams  # REMOVED: torch, accelerate, transformers

MODEL_ID = "meta-llama/Llama-3.3-70B-Instruct"
INPUT_FILE = "kbg_synthetic_conversations_scale_3.jsonl"
OUTPUT_FILE = "kbg_final_patient_reports_scale_3.jsonl"

# Pass your Hugging Face token directly to the environment so vLLM can download Llama 3.3

os.environ["HF_TOKEN"] = "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"
...
def main():
    ...
    llm = LLM(
        model=MODEL_ID,
        tensor_parallel_size=4,        # MUST match your #SBATCH --gpus=4 allocation
        dtype="bfloat16",              # Safe unquantised precision for Isambard's GH200 nodes
        max_model_len=8192             # Plenty of context space for long medical transcripts
    )

    # 2. Configure generation sampling rules

    sampling_params = SamplingParams(
        max_tokens=2048,               # Matches max_new_tokens
        temperature=0.3                # Matches original temperature
    )
    ...
    # 3. Gather transcripts into an entry array list
    ...
    with open(INPUT_FILE, "r", encoding="utf-8") as infile:
        ...
        for line in infile:
            ...
            # Keep track of the transcript to populate the output dataset later
            transcript = record.get("synthetic_transcript", "")
            transcripts_batch.append(transcript)

            # FIXED: Reverted to your exact layout structure with empty content strings
            messages = [
            {
                "role": "system",
                "content": (
    ...
    # 4. Trigger parallel batch synthesis on the GPU grid

    print(f"Starting parallel batch generation for {len(prompts)} records...")
    ...
    # 5. Unpack outputs and write out the structured records

    with open(OUTPUT_FILE, "w", encoding="utf-8") as outfile:

        for idx, output in enumerate(outputs):

            # FIXED: Corrected list index to safely pull the text string from the choice array
            ...
            distillation_record = {
                ...
                "input": transcripts_batch[idx],
                "output": generated_report
            }

            outfile.write(
                json.dumps(distillation_record, ensure_ascii=False) + "\n"
            )
...
MODEL_ID = "meta-llama/Llama-3.3-70B-Instruct"
INPUT_FILE = "kbg_synthetic_conversations_scale_test4.jsonl"
OUTPUT_FILE = "kbg_final_patient_reports_scale_test4.jsonl"
...
3. Identify only the relevant clinical recommendations for each section based on the extracted findings.
4. Generate the final report with section-specific recommendations.
...
- "Recommendations for screening and treatments" must include only section-specific recommendations and must not contain unrelated suggestions.
- Assign each symptom exclusively to its most appropriate clinical category.
- Recommendations must relate only to symptoms reported in the consultation.
...
a) How this affects the patient:
b) How this affects others with the syndrome:
c) Recommendations for screening and treatments:
